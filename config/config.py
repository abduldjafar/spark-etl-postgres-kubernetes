...
class Config(object):
    def __init__(self):
        self.postgres_password = (
            environ["POSTGRES_PASSWORD"] if "POSTGRES_PASSWORD" in environ else ""
        )
        self.postgres_host = (
            environ["POSTGRES_HOST"] if "POSTGRES_HOST" in environ else ""
        )
        self.postgres_port = (
            environ["POSTGRES_PORT"] if "POSTGRES_PORT" in environ else ""
        )
        self.postgres_user = (
            environ["POSTGRES_USER"] if "POSTGRES_USER" in environ else ""
        )
        self.data_sources = (
            environ["DATA_SOURCES"]
            if "DATA_SOURCES" in environ
            else "data/transaction.csv"
        )
        self.database = (
            environ["DATABASE_DESTINATION"] if "DATABASE_DESTINATION" in environ else ""
        )
        self.table = (
