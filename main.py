...
def load_csv_datas():
  df = spark_session().read.csv(init_config().get_data_sources(), sep='|', header=True, inferSchema=True)
  return df
...
def get_greatest_total_units_sold_date_trx(greatest_total_units_sold,mainDatas):
  ...
  return greatest_total_units_sold \
    .join(
      mainDatas,
      [
...
def get_longest_streak(get_date_trx_every_items):
  ...
  return get_date_trx_every_items \
    .select("customer_id","favourite_product","transactionDate") \
    .withColumn(
      "lag",
      lag("transactionDate",1)
        .over(
          Window
          .partitionBy(
            "customer_id",
...
def write_to_rdbms(df,rdbms_type):
  ...
  df.write.format('jdbc') \
    ...
    .option('dbtable',init_config().table) \
    .option('user',init_config().get_postgres_user) \
    .option('password',init_config().get_postgres_password) \
    .save()
